...
beforeEach((done) => {
  ...
  (redisCache.store as Store)
    .getClient()
    .once('ready', () => redisCache.reset(done));
});
...
describe('set', () => {
  ...
  it('should not store undefined', async () => {
    expect.assertions(1);
    await expect(redisCache.set('foo', undefined)).rejects.toEqual(
      Error('undefined is not cacheable')
    );
  });

  it('should not store null', async () => {
    expect.assertions(1);
    await expect(redisCache.set('foo', null)).rejects.toEqual(
      Error('null is not cacheable')
    );
  });
  ...
  it('should not store a value disallowed by isCacheableValue', async () => {
    ...
    await expect(customRedisCache.set('foo', 'FooBarString')).rejects.toEqual(
      Error('FooBarString is not cacheable')
    );
  });
  ...
  it('should return an error if there is an error acquiring a connection', async () => {
    ...
    await expect(redisCache.set('foo', 'bar')).rejects.toEqual(
...
describe('get', () => {
  ...
  it('should reject promise on error', async () => {
    const cache = redisCache.store as Store;
    await cache.getClient().quit();
    await expect(redisCache.get('foo')).rejects.toEqual(
      Error('Connection is closed.')
    );
  });
...
describe('mget', () => {
  it('should retrieve multiple values', async () => {
    ...
    expect(await redisCache.store.mget?.(['foo', 'baz', null])).toEqual([
      'bar',
      'foo',
      null,
    ]);
  ...
  it('should retrieve multiple values with options and varargs', async () => {
    await redisCache.set('foo', 'bar');
    await redisCache.set('baz', 'foo');
    expect(await redisCache.store.mget?.('foo', 'baz', 'bar', {})).toEqual([
      'bar',
      'foo',
      null,
    ]);
  ...
  it('should return an error if there is an error acquiring a connection', async () => {
    const cache = redisCache.store as Store;
    await cache.getClient().quit();
    await expect(cache.mget('foo')).rejects.toEqual(
      Error('Connection is closed.')
    );
  });
});
...
describe('del', () => {
  ...
  it('should return an error if there is an error acquiring a connection', async () => {
    const cache = redisCache.store as Store;
    await cache.getClient().quit();
    await expect(redisCache.del('foo')).rejects.toEqual(
      Error('Connection is closed.')
    );
  });
});
...
describe('reset', () => {
  ...
  it('should return an error if there is an error acquiring a connection', async () => {
    const cache = redisCache.store as Store;
    await cache.getClient().quit();
    await expect(redisCache.reset()).rejects.toEqual(
      Error('Connection is closed.')
    );
  });
});
...
describe('ttl', () => {
  ...
  it('should return an error if there is an error acquiring a connection', async () => {
    const cache = redisCache.store as Store;
    await cache.getClient().quit();
    await expect(cache.ttl('toto')).rejects.toEqual(
      Error('Connection is closed.')
    );
  });
});
...
describe('keys', () => {
  ...
  it('should return an error if there is an error acquiring a connection', async () => {
    const cache = redisCache.store as Store;
    await cache.getClient().quit();
    await expect(cache.keys('toto')).rejects.toEqual(
      Error('Connection is closed.')
    );
  });
});
...
describe('wrap function', () => {
  ...
  function getUser(
    id: number,
    cb: (err: Error | null, user?: { id: number }) => void
  ) {
    setTimeout(() => {
      cb(null, { id: id });
    }, 100);
  ...
  it('should work with promises', async () => {
    ...
    const user = await redisCache.wrap('wrap-promise', () =>
      getUserPromise(userId)
    );
    expect(user.id).toEqual(userId);
    const cached = await redisCache.wrap('wrap-promise', () =>
